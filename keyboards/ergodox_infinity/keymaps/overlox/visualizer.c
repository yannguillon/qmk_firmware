...
#define CROSSFADE_TIME 100
#define GRADIENT_TIME 10000
...
static void get_visualizer_layer_and_color(visualizer_state_t* state) {
    ...
    if (state->status.layer & 0x8) {
      ...
      // violet = 200
        state->target_lcd_color = LCD_COLOR(25, saturation, 0xFF);
        state->layer_text = "ARROWS + F#";
    }
    else {
      // 45
        state->target_lcd_color = LCD_COLOR(46, saturation, 0xFF);
